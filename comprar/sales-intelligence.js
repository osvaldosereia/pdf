(()=>{
  ...
  let current=null,trackedProductId=null,locked=false,attempted=false,rejected=false,lastCartCount=-1,timer=null;
  ...
  async function rejectOffer(){if(!current)return;const offer=current;rejected=true;trackedProductId=null;hide();try{await api('event',{event_type:'rejected',product_id:offer.product_id})}catch{}}
  function schedule(force=false){clearTimeout(timer);timer=setTimeout(()=>refresh(force),1200)}
  document.addEventListener('click',e=>{const plus=e.target.closest?.('.product-card .plus');if(plus){const card=plus.closest('.product-card'),id=card?.dataset?.id;if(trackedProductId&&id===trackedProductId){setTimeout(async()=>{try{await api('event',{event_type:'added',product_id:id});trackedProductId=null;hide()}catch{}},1000)}schedule(true)}else if(e.target.closest?.('.basket-select,.basket-item .plus,.basket-item .minus,.product-card .minus'))schedule(true)},true);
  ...
  window.addEventListener('beforeunload',()=>{if(trackedProductId&&!rejected)navigator.sendBeacon?.(C.salesApi,new Blob([JSON.stringify({action:'event',token,event_type:'ignored',product_id:trackedProductId})],{type:'application/json'}))});
  schedule();
})();
