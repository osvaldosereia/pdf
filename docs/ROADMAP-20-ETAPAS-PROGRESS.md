...
**PRÓXIMA ETAPA.** Implementar o motor `TRIGGER → CONDITIONS → ACTIONS` referenciando exclusivamente ações governadas da Etapa 9, com versionamento, dry-run/simulador, budgets/cooldown/kill switch, estratégias de execução e Builder no Admin; tudo desligado por padrão e sem side effect real na fundação inicial.
