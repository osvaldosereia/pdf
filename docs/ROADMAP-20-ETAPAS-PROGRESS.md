...
**EM IMPLEMENTAÇÃO — BLOCOS 1, 2 E 3 PROGRAMÁVEIS CONCLUÍDOS E DORMENTES.** A PR #197 criou a fundação server-only `TRIGGER → CONDITIONS → ACTIONS`, com `automation_workflows`, versões, execuções/eventos, RLS, acesso service-role, simulador sem efeitos e recomendador GitHub Actions first. A migration real `automation_engine_stage10_v1` segue aplicada e não foi alterada nesta rodada.

A PR #198 foi integrada no `main` (`3e82abbf5fa4138a6f15f85d5c66ceac6a7d8dcd`) e concluiu o segundo bloco seguro: o Automation Builder está montado no Admin, porém permanece invisível atrás de `automationBuilderUiEnabled=false`; visualiza `TRIGGER → CONDITIONS → ACTIONS`, valida e simula workflows e só oferece mutação operacional unilateralmente segura de kill switch. O compilador linguagem natural → workflow draft segue não persistente e revisável, com fallback determinístico sem custo externo. Conditions usam allowlist estruturada de campos e operadores; drafts continuam forçados a `enabled=false`, `execution_mode=off`, `canary_percent=0`, `kill_switch=true`.

A PR #199 foi integrada no `main` (`1098218ee6d14834ebfdcea6420d77ee7a92553c`) e concluiu o terceiro bloco: foi criado `automation-dispatcher-v1`, estritamente limitado a workflows em `observe`/`dry_run`, sem qualquer executor de action ou chamada externa. Antes de registrar uma tentativa ele revalida JWT/Admin, workflow, `enabled`, kill switch, handoff humano por `conversation_id`, budget `max_runs_per_hour`, idempotência e o AI Action Registry por meio de `simulate_automation_workflow_v1`. Cada tentativa fica auditada em `automation_workflow_executions` e `automation_workflow_events` com `external_side_effect=false`, custo estimado/real `0` e replay idempotente. `live` é explicitamente não suportado e nenhum HTTP externo/OpenAI é chamado pelo dispatcher. `supabase/config.toml` agora fixa `verify_jwt=true` para `admin-automation-builder-v1` e `automation-dispatcher-v1`.

O CI específico `Stage 10 Automation Engine` foi ampliado para v1/v2/v3 e ficou verde, incluindo teste do contrato fail-closed e `deno check` do Builder e do dispatcher. O dispatcher não foi publicado/deployado nesta rodada porque isso alteraria o runtime de produção; o código está versionado e dormente no repositório. Nenhuma migration nova foi necessária.

Auditoria pós-merge confirmou no Supabase: `workflows=2`, `enabled_workflows=0`, `non_off_workflows=0`, `actions=9`, `enabled_actions=0`, `non_off_actions=0`, `open_handoffs=3`; `whatsapp_release_mode=live`, `whatsapp_live_canary_percent=1`, `experience_orchestrator_enabled=false`, `whatsapp_flow_data_exchange_enabled=false`, `whatsapp_flow_send_enabled=false`. Make segue com inbound `6779824` e outbound event-driven `7290488` ativos como pontes realtime; nenhuma automação geral nova foi criada no Make.

**Próximo bloco da própria Etapa 10:** preparar o compilador OpenAI real atrás de gate explícito, schema estrito, teto de custo, observabilidade e revisão humana antes de persistir; manter fallback determinístico como caminho sem custo. Só considerar a Etapa 10 programaticamente concluída depois que essa integração estiver pronta e testada sem ativação automática. Não iniciar Etapa 11 antes disso.
