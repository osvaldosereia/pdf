...
**EM IMPLEMENTAÇÃO — BLOCO 1 PROGRAMÁVEL CONCLUÍDO E DORMENTE.** PR #197 prepara a fundação server-only `TRIGGER → CONDITIONS → ACTIONS` com `automation_workflows`, versões, execuções e eventos, todos protegidos por RLS e acesso service-role. A migration real `automation_engine_stage10_v1` foi aplicada. Readiness: `workflows=2`, `enabled_workflows=0`, `non_off_workflows=0`, `kill_switch_off=0`; templates iniciais permanecem `enabled=false`, `execution_mode=off`, `canary_percent=0`, `kill_switch=true`. Actions são validadas exclusivamente contra o AI Action Registry e o simulador não executa side effects. Foi criado recomendador de estratégia com GitHub Actions first para batch/periódico determinístico e justificativa obrigatória para Make. Edge Function administrativa e Builder visual estão versionados, sem caminho de ativação e com UI `automationBuilderUiEnabled=false`; o deploy da nova Edge Function foi bloqueado pelo controle de segurança da ferramenta e não foi contornado. CI específico da Etapa 10 ficou verde; regressão do conversation worker também ficou verde. Um workflow legado de validação geral do Admin falhou somente no passo agregador `Confirmar resultado`, após todas as validações técnicas individuais exibidas no job terem passado; deve ser investigado sem sacrificar os gates desta etapa. Próximo bloco da própria Etapa 10: integrar/montar o Builder no Admin ainda atrás de flag, adicionar compilador linguagem natural → workflow draft revisável, enriquecer conditions e preparar dispatcher exclusivamente dry-run/observe. Não avançar à Etapa 11 até concluir esse bloco programável seguro.
