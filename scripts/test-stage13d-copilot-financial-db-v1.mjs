...
try{
  await db.exec(`
    ...
    create table public.conversations(id uuid primary key default gen_random_uuid(),customer_id uuid references public.customers(id),channel text not null default 'whatsapp',stage text not null default 'new',mode text not null default 'human',status text not null default 'needs_human',service_window_expires_at timestamptz,human_required boolean not null default true,updated_at timestamptz default now());
