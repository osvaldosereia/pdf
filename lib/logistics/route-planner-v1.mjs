...
export function buildDraftRoutePlan(jobs,{vehicle=null,driver=null,maxStops=null}={}){
  ...
  const missingCoordinates=rows.filter(j=>{
    const lat=coordinate(j.latitude),lng=coordinate(j.longitude);
    return lat===null||lng===null||lat < -90||lat > 90||lng < -180||lng > 180||['required','blocked'].includes(j.geocode_status);
  });
  ...
  return {ok:true,status:'draft',geographically_optimized:false,requires_provider_optimization:true,driver:driver?{id:driver.id}:null,vehicle:vehicle?{id:vehicle.id}:null,stops:ordered.map((j,index)=>({sequence_no:index+1,delivery_job_id:j.id,latitude:coordinate(j.latitude),longitude:coordinate(j.longitude),priority:j.priority,window_start:j.delivery_window_start||null,window_end:j.delivery_window_end||null,locked:false})),side_effect_performed:false};
}
