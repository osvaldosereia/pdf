...
export function buildPickingPlan(items=[]){
  ...
  const normalized=items.map((item,index)=>({
    id:text(item.id||index),product_id:text(item.product_id),name:text(item.name),gtin:text(item.gtin),quantity:num(item.quantity),
    location_code:text(item.location_code),gondola_code:text(item.gondola_code),shelf_code:text(item.shelf_code),
    pick_sequence:Number.isFinite(num(item.pick_sequence))?num(item.pick_sequence):1000,
    expires_at:text(item.expires_at),lot_code:text(item.lot_code),
  }));
...
export function chooseFefoLot(lots=[],deliveryDate=new Date().toISOString().slice(0,10)){
  const valid=(Array.isArray(lots)?lots:[]).filter(l=>text(l.status)==='available'&&l.physically_verified!==false&&num(l.quantity_available)>num(l.quantity_reserved)&&(!text(l.expires_at)||text(l.expires_at)>=deliveryDate));
  valid.sort((a,b)=>text(a.expires_at||'9999-12-31').localeCompare(text(b.expires_at||'9999-12-31'))||text(a.received_at||'9999-12-31').localeCompare(text(b.received_at||'9999-12-31'))||text(a.lot_code).localeCompare(text(b.lot_code)));
  return valid[0]||null;
}
