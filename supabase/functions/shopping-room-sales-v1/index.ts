...
Deno.serve(async(req:Request)=>{
  ...
  if(action==='next_offer'){
    ...
    if(recent?.product_id){
      const {data:response}=await sb.from('sales_offer_events').select('id,event_type').eq('conversation_id',session.conversation_id).eq('product_id',recent.product_id).in('event_type',['added','rejected','ignored']).gt('occurred_at',recent.occurred_at).order('occurred_at',{ascending:false}).limit(1).maybeSingle();
      if(!response){const {data:p}=await sb.from('products').select('id,name,price,image_url,brand,packaging,sales_category,stock,is_offer').eq('id',recent.product_id).eq('is_active',true).eq('is_whatsapp_active',true).gt('stock',0).maybeSingle();if(p)return json(req,{ok:true,action:'offer_suggestions',offer:{...p,reason:clean(recent.context?.reason||'',200)},event_id:recent.id,reused:true});}
    }
    const {data:plan,error}=await sb.rpc('plan_next_sales_move',{p_conversation_id:session.conversation_id});if(error)return json(req,{ok:false,error:'sales_plan_failed'},500);
  ...
  if(action==='event'){
    const eventType=clean(body?.event_type,30),productId=clean(body?.product_id,80);if(!['viewed','added','rejected','ignored'].includes(eventType))return json(req,{ok:false,error:'invalid_sales_event'},400);if(!validUuid(productId))return json(req,{ok:false,error:'invalid_product'},400);
    ...
    const dedupeSince=new Date(Date.now()-5*60*1000).toISOString();const {data:duplicate}=await sb.from('sales_offer_events').select('id').eq('conversation_id',session.conversation_id).eq('product_id',productId).eq('event_type',eventType).gte('occurred_at',dedupeSince).limit(1).maybeSingle();if(duplicate)return json(req,{ok:true,event:{id:duplicate.id,event_type:eventType},duplicate:true});
