...
export function evaluateInstagramPrivateReplyPolicyV1({ commentCreatedAt, isLive=false, liveStateVerifiedActive=false, alreadyReplied=false, now=new Date() }={}) {
...
export function buildInstagramPrivateReplyDraftV1(intent) {
  ...
  if(value==='purchase_interest') return 'Oi! Vi seu comentário. Posso te ajudar por aqui com opções e disponibilidade. Se quiser continuar, responda esta mensagem.';
  if(value==='question') return 'Oi! Vi sua dúvida no comentário. Posso te explicar por aqui. Se quiser continuar, responda esta mensagem.';
  if(value==='support') return 'Oi! Vi seu comentário e quero entender melhor para ajudar. Se puder, responda esta mensagem com os detalhes.';
