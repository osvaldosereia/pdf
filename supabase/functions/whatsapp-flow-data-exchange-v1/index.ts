...
import {decryptFlowRequest,encryptFlowResponse,FlowCryptoError,type EncryptedFlowEnvelope} from "./crypto.ts";
...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    const screen=text(body.screen,120)||null;
    ...
    if(action==="ping"){
      ...
      await sb.from("whatsapp_flow_exchange_events").insert({request_id:requestId,action:"ping",screen:null,status:"accepted"});
      ...
      if(sessionId){
        await sb.rpc("record_whatsapp_flow_exchange_v1",{p_session_id:sessionId,p_request_id:requestId,p_action:action||"client_error",p_screen:screen,p_status:eventStatus,p_error_code:errorCode});
      }else{
        await sb.from("whatsapp_flow_exchange_events").insert({request_id:requestId,action:action||"client_error",screen,status:eventStatus,error_code:errorCode});
      }
    }else{
      ...
      sessionId=handled?.session_id||null;
      if(!handled?.ok){
        eventStatus="rejected";
        errorCode=text(handled?.reason,120)||"flow_rejected";
        response={data:{error:true,error_code:errorCode}};
      }else{
      ...
      if(sessionId){
        await sb.rpc("record_whatsapp_flow_exchange_v1",{p_session_id:sessionId,p_request_id:requestId,p_action:action||"unknown",p_screen:screen,p_status:eventStatus,p_error_code:errorCode});
      }else{
        await sb.from("whatsapp_flow_exchange_events").insert({request_id:requestId,action:action||"unknown",screen,status:eventStatus,error_code:errorCode});
      }
    }
    ...
    // Never log decrypted payloads, tokens, keys or customer data.
