...
const nullableNonNegativeInt=(v:unknown)=>{if(v==null||v==="")return null;const n=Number(v);return Number.isInteger(n)&&n>=0?n:NaN;};
const nullableIso=(v:unknown)=>{const s=clean(v,80);if(!s)return null;const d=new Date(s);return Number.isNaN(d.getTime())?undefined:d.toISOString();};
...
Deno.serve(async(req:Request)=>{
  ...
  if(!admin?.is_active||admin.role!=="owner")return json({ok:false,error:"owner_required"},403);
  ...
  if(action==="policies"){
    ...
    return json({ok:true,policies:data.items||[]});
  }
  ...
  if(action==="approve_policy"){
    ...
    const {data,error}=await sb.rpc("approve_financial_policy_v1",{p_policy_id:policyId,p_admin_user_id:user.id,p_effective_from:effectiveFrom||new Date().toISOString(),p_reason:clean(body.reason,1000)||null});
  ...
  // Deliberadamente ausentes: apply_projection, ledger_write, provider_ingest e fiscal_issue.
