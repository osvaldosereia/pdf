...
Deno.serve(async(req:Request)=>{
  ...
  if(action==='create_catalog'||action==='create_room'){
    ...
    const {data:session,error}=await sb.rpc('create_customer_catalog_session',{p_customer_id:id,p_conversation_id:body?.conversation_id||null,p_cart_id:body?.cart_id||null,p_kind:kind,p_limit:limit,p_created_by:user.id});if(error)return json({ok:false,error:'room_create_failed',detail:error.message},400);
    ...
    return json({ok:true,session,link,room_url:link,whatsapp_text:whatsappText,voice_intro:voiceIntro,item_count:session.item_count||0});
  }
