...
-- Corrige falha runtime encontrada na homologação observe real.
-- queue_human_handoff_v1 recebe p_priority smallint; o literal inteiro sem cast
-- fazia o PostgreSQL abortar o ingest inteiro com function does not exist.
...
begin
  ...
  if coalesce((v_result->>'ok')::boolean,false)=true
    ...
    if v_conversation is not null then
      update public.conversations
         set automation_cohort=v_cohort,
             automation_bucket=v_bucket,
             updated_at=now()
    ...
    if coalesce((v_release->>'auto_reply_allowed')::boolean,false) is not true then
      if v_ai_job is not null then
        update public.ai_jobs
           set status='held',
               error_message='release_human_control',
               updated_at=now()
         where id=v_ai_job
      ...
      if v_conversation is not null then
        perform public.queue_human_handoff_v1(
          v_conversation,
          coalesce(v_release->>'reason','release_human_control'),
          v_message,
          2::smallint,
          'Atendimento retido pela liberação gradual.',
          jsonb_build_object(
            'release_mode',v_release->>'mode',
            'cohort',v_cohort,
            'bucket',v_bucket
  ...
  -- Preserva o menu determinístico já homologado para saudações/comando menu.
  if coalesce((v_result->>'ok')::boolean,false)=true
    ...
    if v_body in ('oi','oii','oiii','ola','olaa','olaaa','bom dia','boa tarde','boa noite','menu','inicio','iniciar','comecar') then
      ...
      if coalesce(v_cfg.whatsapp_auto_reply_enabled,false)
         ...
         and v_cfg.whatsapp_release_mode in ('homologation','live') then
...
-- Deploy nunca mantém janela de homologação aberta.
