...
begin
  ...
  select m.ai_interpretation->>'action_type' into v_action from public.messages m where m.id=v_reply_id;
  ...
  if not v_has_address and c.customer_id is not null then
    select exists(select 1 from public.customer_addresses a where a.customer_id=c.customer_id and a.is_active=true and coalesce(a.street,'')<>'' and coalesce(a.number,'')<>'' and coalesce(a.city,'')<>'') into v_has_address;
  end if;
  ...
  on conflict(conversation_id) do update set awaiting='delivery_address',last_action='request_address',updated_at=now();
  update public.messages
     set body_text=v_prompt,message_type='text',
         ai_interpretation=coalesce(ai_interpretation,'{}'::jsonb)||jsonb_build_object('action_type','request_address','delivery_mode','text','checkout_address_first',true)
   where id=v_reply_id;
  ...
  values('checkout_address_requested_before_confirmation','info',c.id,jsonb_build_object('reply_message_id',v_reply_id,'flow','address_before_single_confirmation'));
  return new;
end $$;
