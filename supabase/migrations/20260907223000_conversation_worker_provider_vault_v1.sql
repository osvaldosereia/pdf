...
-- A chave do provider nunca fica no repositório nem em tabelas públicas.
-- Instalação e leitura são service-role only; o hash server-only detecta divergência do Vault.
...
begin
  ...
  if v_key is null or encode(digest(v_key,'sha256'),'hex') is distinct from v_expected_hash then
    return null;
  end if;
...
create or replace function public.get_conversation_worker_provider_health_v1()
returns jsonb
language sql
security definer
set search_path=''
as $$
  select jsonb_build_object(
    'provider','openai',
    'configured',public.get_conversation_worker_provider_secret_v1() is not null,
    'rotated_at',(select rotated_at from public.system_secrets where key_name='openai_conversation_worker_v1')
  );
$$;
...
revoke all on function public.get_conversation_worker_provider_health_v1() from public,anon,authenticated;
...
grant execute on function public.get_conversation_worker_provider_health_v1() to service_role;
