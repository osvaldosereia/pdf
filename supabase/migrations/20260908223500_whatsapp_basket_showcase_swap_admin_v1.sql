...
-- Dona Antônia — vitrine externa dinâmica para personalização e substituição de cestas.
-- O layout da vitrine é fixo (mobile/3 colunas). O Admin controla somente
-- decisões comerciais: categoria disponível, nome exibido e ordem.
...
-- Preserva a ordem original das colunas da view para permitir CREATE OR REPLACE.
...
select c.id,
       ...
       c.basket_showcase_enabled,
       coalesce(nullif(trim(c.basket_showcase_label),''),c.name) as basket_showcase_label,
       c.basket_showcase_sort_order
from public.product_categories c
left join public.products p
  on lower(trim(coalesce(p.category,'')))=lower(trim(c.name))
group by c.id,c.name,c.created_at,c.updated_at,
         c.basket_showcase_enabled,c.basket_showcase_label,c.basket_showcase_sort_order;
...
-- Mantém exatamente o contrato existente de 3 colunas, mudando apenas filtro/ordem.
...
as $$
  ...
  join public.products p
    on lower(trim(coalesce(p.category,'')))=lower(trim(pc.name))
...
begin
  ...
  foreach cat in array coalesce(p_categories,'{}'::text[]) loop
    ...
    if cat<>''
       and exists(
         ...
         where lower(trim(pc.name))=lower(cat) and pc.basket_showcase_enabled=true
       )
       and not exists(select 1 from unnest(selected) x where lower(x)=lower(cat))
  ...
  update public.catalog_sessions
   ...
   where conversation_id=c.id and status='open'
     and metadata->>'flow'='basket_replace_v1';

  insert into public.catalog_sessions(
    customer_id,conversation_id,cart_id,kind,title,status,expires_at,metadata,
    experience,current_view,last_activity_at
  ) values(
    c.customer_id,c.id,base_s.cart_id,'browse','Trocar '||src_name,'open',now()+interval '24 hours',
    jsonb_build_object(
      'flow','basket_replace_v1',
      'parent_basket_session_id',base_s.id,
      'source_product_id',p_source_product_id,
      'source_product_name',src_name,
      'target_query',nullif(q,''),
      'categories',to_jsonb(selected)
    ),
    'shopping_room','replacement',now()
  ) returning * into swap_s;
  ...
  if q<>'' then
    ...
    left join public.product_categories pc
      on lower(trim(pc.name))=lower(trim(coalesce(s.category,'')))
    where s.id<>p_source_product_id
      and coalesce(pc.basket_showcase_enabled,true)=true
      and (cardinality(selected)=0 or exists(select 1 from unnest(selected) x where lower(x)=lower(coalesce(s.category,''))));
    get diagnostics cnt=row_count;
  elsif cardinality(selected)>0 then
    insert into public.catalog_session_items(
      catalog_session_id,product_id,rank,reason,recommendation_score,quantity,metadata
    )
    select swap_s.id,p.id,
           row_number() over(order by pc.basket_showcase_sort_order,p.sort_order nulls last,p.name)::integer,
    ...
    join public.product_categories pc
      on lower(trim(pc.name))=lower(trim(coalesce(p.category,'')))
    where pc.basket_showcase_enabled=true
      and exists(select 1 from unnest(selected) x where lower(x)=lower(coalesce(p.category,'')))
      and p.id<>p_source_product_id
      and p.physically_verified=true
      and p.is_active=true
  ...
  return jsonb_build_object(
    'session_id',swap_s.id,
    'token',swap_s.public_token,
    'source_product_id',p_source_product_id,
    'source_product_name',src_name,
    'target_query',nullif(q,''),
    'categories',to_jsonb(selected),
...
begin
  ...
  update public.catalog_sessions
     set metadata=jsonb_set(metadata,'{categories}',to_jsonb(selected),true),
         current_view='replacement',last_activity_at=now()
   where id=s.id;
...
declare
  s public.catalog_sessions%rowtype;
  parent_s public.catalog_sessions%rowtype;
  parent_id uuid;
  src uuid;
  src_name text;
...
begin
  ...
  if not exists(
    select 1 from public.catalog_session_items
    where catalog_session_id=s.id and product_id=p_replacement_product_id
  ) then raise exception 'replacement_not_in_showcase'; end if;

  select * into repl
  from public.products
  where id=p_replacement_product_id
    and physically_verified=true and is_active=true
    and price is not null and price>=0 and coalesce(stock,0)>0;
  ...
  src:=nullif(s.metadata->>'source_product_id','')::uuid;
  src_name:=coalesce(s.metadata->>'source_product_name','Produto');
  select * into parent_s from public.catalog_sessions where id=parent_id;
  if not found then raise exception 'parent_basket_session_unavailable'; end if;

  substitution:=jsonb_build_object(
    'source_product_id',src,
    'source_name',src_name,
    'replacement_product_id',repl.id,
    'replacement_name',repl.name,
    'replacement_category',repl.category,
    'requested_at',now(),
    'commercial_price_unchanged',true,
  ...
  update public.catalog_sessions
     set metadata=metadata||jsonb_build_object('selected_replacement',substitution),
         current_view='replacement_selected',last_activity_at=now()
   where id=s.id;

  update public.catalog_sessions
     set metadata=metadata||jsonb_build_object('basket_customized',true),last_activity_at=now()
   where id=parent_id;

  return jsonb_build_object(
    'ok',true,
    'substitution',substitution,
    'basket_price_unchanged',true,
    'parent_token',parent_s.public_token,
    'parent_url','https://donaantonia.com.br/cesta/?t='||parent_s.public_token||'&swap=ok'
  );
end $$;
...
begin
  select coalesce(jsonb_agg(
    case
      when i.metadata ? 'substitution'
        then e || jsonb_build_object('substitution',i.metadata->'substitution')
      else e
    end
    order by ord
  ),'[]'::jsonb)
  into enriched
  from jsonb_array_elements(coalesce(new.basket_selection,'[]'::jsonb))
       with ordinality a(e,ord)
  left join public.catalog_session_items i
    on i.catalog_session_id=new.basket_session_id
   and i.product_id=case
     when coalesce(e->>'product_id','') ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'
       then (e->>'product_id')::uuid
     else null
   end;
...
drop trigger if exists trg_enrich_whatsapp_basket_order_substitutions_v1
  on public.whatsapp_basket_order_requests;
...
-- Intercepta pedidos explícitos de troca antes do worker livre.
-- A substituição em si nunca acontece no WhatsApp: o chat apenas gera o link.
...
begin
  ...
  if position(' por ' in norm)>0 then
    source_part:=trim(regexp_replace(split_part(norm,' por ',1),
      '^.*?(trocar|troca|substituir|substitui|mudar|muda)\s+','','i'));
    target_part:=trim(split_part(norm,' por ',2));
  else
    source_part:=trim(regexp_replace(norm,
      '^.*?(trocar|troca|substituir|substitui|mudar|muda)\s+','','i'));
  end if;
  ...
  if source_part<>'' then
    ...
    where i.catalog_session_id=bs.id
      and translate(lower(p.name),
        'áàãâäéèêëíìîïóòõôöúùûüç',
        'aaaaaeeeeiiiiooooouuuuc') like '%'||source_part||'%'
    order by length(p.name),i.rank
    limit 1;
  end if;

  if src is null then
    reply:='A troca é feita na sua cesta, fora do WhatsApp. Abra a cesta e toque em “Trocar” no produto que deseja substituir:'||E'\n'||
    ...
    perform public.queue_whatsapp_sales_reply_v1(
      ...
      'basket_swap_open_showcase',jsonb_build_object('deterministic',true),1
    );
  else
...
-- Regressões para garantir que troca nunca seja executada dentro do WhatsApp.
delete from public.service_regression_cases
where case_key in ('basket_swap_external_showcase_v1','basket_swap_unknown_target_categories_v1');
insert into public.service_regression_cases(
  case_key,title,customer_message,setup,expected_intent,expected_action,
  expected_assertions,status,priority
) values
(
  'basket_swap_external_showcase_v1',
  'Troca de cesta abre vitrine externa',
  'Troca o arroz por feijão',
  jsonb_build_object('basket_selected',true),
  'basket_swap','basket_swap_showcase',
  jsonb_build_object('must_use_external_showcase',true,'must_not_replace_in_whatsapp',true,'same_cart',true),
  'active',100
),
(
  'basket_swap_unknown_target_categories_v1',
  'Troca sem substituto pede categoria na vitrine',
  'Quero trocar o arroz',
  jsonb_build_object('basket_selected',true),
  'basket_swap','basket_swap_showcase',
  jsonb_build_object('must_offer_real_categories',true,'must_not_replace_in_whatsapp',true),
  'active',100
);
