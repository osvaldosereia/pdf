(function(){
  ...
  let root=null,last=null,lastFlow=null,loading=false;
  ...
  async function callFunction(fn,action,payload={},retry=true){
    ...
    const data=await r.json().catch(()=>({}));
    if(r.status===401&&retry){a=await refreshToken(a);return callFunction(fn,action,payload,false)}
    if(!r.ok||data.ok===false)throw new Error(data.detail||data.error||`Erro ${r.status}`);
    return data;
  }
  const api=(action,payload={},retry=true)=>callFunction(C.experienceOrchestratorEdgeFunction||'admin-experience-orchestrator-v1',action,payload,retry);
  const flowApi=(action,payload={},retry=true)=>callFunction(C.whatsappFlowAdminEdgeFunction||'admin-whatsapp-flow-v1',action,payload,retry);
  ...
  function shell(){return `
    <section class="exp-shell">
      ...
      <div class="exp-grid">
        ...
        <section class="exp-card"><header><div><small>Chaves / transporte</small><h3>Ciclo de vida da chave Flow</h3></div><span class="exp-badge muted">runtime fechado</span></header><div id="expFlowKeyLifecycle" class="exp-list"><div class="empty">Carregando…</div></div></section>
      </div>
      <div class="exp-grid">
  ...
  function renderFlowAdmin(data){
    lastFlow=data;
    const box=root.querySelector('#expFlowKeyLifecycle');if(!box)return;
    const readiness=data?.readiness||{};
    const config=data?.config||{};
    const role=String(data?.user?.role||last?.user?.role||'');
    const owner=role==='owner';
    const publicKey=String(config.public_key_pem||'').trim();
    const fingerprint=String(config.public_key_fingerprint||readiness.public_key_fingerprint||'').trim();
    const transportOff=!readiness.data_exchange_enabled&&!readiness.send_enabled;
    const keyBadge=readiness.private_key_configured&&readiness.public_key_configured?'<span class="exp-badge success">configurada</span>':'<span class="exp-badge muted">não configurada</span>';
    const ownerControls=owner?`
      <div class="exp-row"><div><strong>Operações de chave</strong><small>Gerar/rotacionar só é permitido com transporte desligado e confirmação literal. A private key vai direto para o Vault.</small></div><div><button class="row-button" data-flow-key-generate type="button" ${transportOff?'':'disabled'}>${readiness.private_key_configured?'Rotacionar chave':'Gerar par RSA'}</button></div></div>
      <div class="exp-row"><div><strong>Kill switch de transporte</strong><small>Ação unilateral de segurança: somente desliga Data Exchange/envio; nunca liga.</small></div><div><button class="row-button" data-flow-disable type="button">Desligar transporte</button></div></div>`:
      '<div class="exp-row"><div><strong>Operações de chave</strong><small>Somente owner pode gerar/rotacionar. Este painel não oferece ativação do transporte.</small></div><span class="exp-badge muted">somente leitura</span></div>';
    box.innerHTML=`
      <div class="exp-row"><div><strong>Estado da chave</strong><small>Versão ${n(config.key_version||readiness.key_version||0)} · fingerprint ${esc(fingerprint||'—')}</small></div>${keyBadge}</div>
      <div class="exp-row"><div><strong>Status Meta</strong><small>Registro/assinatura da chave pública é etapa externa separada.</small></div>${statusBadge(config.meta_signature_status||readiness.meta_signature_status||'unknown')}</div>
      <div class="exp-row"><div><strong>Runtime</strong><small>Data Exchange ${readiness.data_exchange_enabled?'ON':'OFF'} · envio ${readiness.send_enabled?'ON':'OFF'} · orquestrador ${readiness.orchestrator_enabled?'ON':'OFF'}</small></div>${transportOff?'<span class="exp-badge success">fechado</span>':'<span class="exp-badge warning">revisar</span>'}</div>
      ${publicKey?`<div class="exp-row"><div><strong>Chave pública</strong><small>Pode ser copiada para registro posterior na Meta. A chave privada nunca aparece aqui.</small></div><button class="row-button" data-flow-copy-public type="button">Copiar pública</button></div><pre class="exp-preview" id="expFlowPublicKey">${esc(publicKey)}</pre>`:'<div class="exp-row"><div><strong>Chave pública</strong><small>Nenhum par foi gerado. Nada precisa ser copiado para a Meta nesta fase.</small></div><span class="exp-badge muted">ausente</span></div>'}
      ${ownerControls}`;
  }

  async function load(){
    if(!root||loading)return;loading=true;
    try{
      const data=await api('dashboard');render(data);
      try{renderFlowAdmin(await flowApi('dashboard'))}
      catch(e){const box=root.querySelector('#expFlowKeyLifecycle');if(box)box.innerHTML=`<div class="empty">${esc(e.message)}</div>`}
    }catch(e){
      toast(e.message,'error');const features=root.querySelector('#expFeatures');if(features)features.innerHTML=`<div class="empty">${esc(e.message)}</div>`;
  ...
  async function generateFlowKey(){
    if(String(lastFlow?.user?.role||'')!=='owner')return toast('Somente owner pode gerar ou rotacionar a chave Flow.','error');
    const confirmation=window.prompt('Esta operação substitui o par atual e invalida a assinatura anterior na Meta. Digite exatamente GERAR_CHAVE_FLOW para continuar:','');
    if(confirmation===null)return;
    if(confirmation!=='GERAR_CHAVE_FLOW')return toast('Confirmação inválida. Nenhuma chave foi alterada.','error');
    try{
      const data=await flowApi('generate_keypair',{confirmation});
      if(data.private_key_pem||data.privateKey||data.private_key_returned!==false)throw new Error('Falha de segurança: resposta inesperada do ciclo de chave.');
      toast('Par RSA gerado no backend. A chave privada permaneceu no Vault.','success');
      await load();
    }catch(e){toast(e.message,'error')}
  }
  async function copyFlowPublicKey(){
    const publicKey=String(lastFlow?.config?.public_key_pem||'').trim();
    if(!publicKey)return toast('Nenhuma chave pública disponível.','error');
    if(!navigator.clipboard?.writeText)return toast('Cópia automática não disponível neste navegador.','error');
    try{await navigator.clipboard.writeText(publicKey);toast('Chave pública copiada.','success')}catch{return toast('Não foi possível copiar a chave pública.','error')}
  }
  async function disableFlowTransport(){
    if(String(lastFlow?.user?.role||'')!=='owner')return toast('Somente owner pode alterar o transporte Flow.','error');
    if(!window.confirm('Desligar Data Exchange e envio de Flow? Esta ação não liga nenhum recurso e é segura para emergência.'))return;
    try{await flowApi('disable_transport');toast('Transporte Flow confirmado como desligado.','success');await load()}catch(e){toast(e.message,'error')}
  }

  function bind(){
    root.querySelector('#expReload').addEventListener('click',load);
    root.querySelector('#expPreviewForm').addEventListener('submit',preview);
    root.addEventListener('click',e=>{const b=e.target.closest('button');if(!b)return;if(b.dataset.expFeature)editFeature(b.dataset.expFeature);if(b.dataset.expDefinition)editDefinition(b.dataset.expDefinition);if(b.hasAttribute('data-flow-key-generate'))generateFlowKey();if(b.hasAttribute('data-flow-copy-public'))copyFlowPublicKey();if(b.hasAttribute('data-flow-disable'))disableFlowTransport()});
  }
